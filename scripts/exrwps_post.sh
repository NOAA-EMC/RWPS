...
# 0.c Defining model grids
  ...
  export ngrids='rwps' # Grids with netcdf output
...
# 1.b Model definition files
  ...
  do
    ...
    then
      ...
      $USHrwps/wavemod_def.sh $grdID > $grdID.out
...
# 1.c Raw point data file and moddef file
  ...
  if [ -f "$FIXrwps/wave_${modID}_${pntgrd}.moddef.${rwps_ver}" ]
  then
    set +x
    echo " Mod def file for $pntgrd found in $FIXrwps. copying ...."
    [[ "$LOUD" = YES ]] && set -x
    cp $FIXrwps/wave_${modID}_${pntgrd}.moddef.${rwps_ver} mod_def.$pntgrd
  else
    set +x
    echo " Mod def file for $pntgrd not found in $FIXrwps. Setting up to generate ..."
    [[ "$LOUD" = YES ]] && set -x
    if [  -f $FIXrwps/wave_${pntgrd}.inp ]
    then 
       cp $FIXrwps/wave_$pntgrd.inp $pntgrd.inp
    fi 

    if [  -f $pntgrd.inp ]
    then
      set +x
      echo "   $pntgrd.inp copied ($FIXrwps/wave_$pntgrd.inp)."
      [[ "$LOUD" = YES ]] && set -x
    else
      msg="ABNORMAL EXIT: NO INP FILE FOR MODEL DEFINITION FILE"
      postmsg   "$msg"
    ...
    $USHrwps/wavemod_def.sh $pntgrd > $pntgrd.out
...
# 1.d Output locations file
  ...
  then
    cp $FIXrwps/wave_$modID.buoys buoy.loc.temp
  ...
  then
    set +x
    echo "   buoy.loc copied and processed ($FIXrwps/wave_$modID.buoys)."
    [[ "$LOUD" = YES ]] && set -x
  else
    set +x
    echo ' '
...
# 2.c.2 Create netcdf gridded file
  ...
  then
    ...
    for grdID in $ngrids
    do
      echo "$USHrwps/wave_fldn.sh $grdID $dtnc $ncdfFL > ncdf_$grdID.out 2>&1"               >> cmdfile
    done
  fi
