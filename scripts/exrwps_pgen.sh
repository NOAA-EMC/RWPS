...
# 0.a Basic modes of operation
  ...
  then
    grids='grlc_2p5km grlc_2p5km_sr'
  else
    grids='grlc_2p5km_sr'
  fi
  awipsgrib='yes'
...
# Unified extension as now there is only one run type
  ...
  do
    ...
    if [ -f $PARMrwps/grib2_awips_${ext}.$grdID ]
    then
      cp $PARMrwps/grib2_awips_${ext}.$grdID awipsgrb.$grdID
    fi
  ...
  if [ "$awipsbull" = 'yes' ]
  then 
    if [ -f $PARMrwps/bull_awp$modID ]
    then
      cp $PARMrwps/bull_awp$modID awipsbull.data
    fi
...
# test version
#     $HOMErwps/scripts/formbul.pl -d $headr -f $fname -j $job -m $modID \
...
# operational version
      ...
      cat $oname >> rwps.t${cyc}z.awipsbull
      rm -f formbul.out $oname
    done

  if [ $SENDCOM = "YES" ] ; then
    #XXW cp  awipsbull.$job $COMOUTwmo/awipsbull.$job
    cp  rwps.t${cyc}z.awipsbull $COMOUTwmo/.
    if [ $SENDDBN = "YES" ] ; then
       #XXW make_ntc_bull.pl  WMOBH NONE KWBJ NONE $DATA/awipsbull.$job $COMOUTwmo/awipsbull.$job
       make_ntc_bull.pl  WMOBH NONE KWBJ NONE $DATA/rwps.t${cyc}z.awipsbull $COMOUTwmo/rwps.t${cyc}z.awipsbull
    fi
  fi
